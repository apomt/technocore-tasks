"""Verify the bounded Phase 1 tclk job against a manifest-marked synthetic database only."""
...
def write_markdown(path: Path, result: dict[str, object]) -> None:
    ...
    lines = [
        "# Technocore Tasks Phase 1 independent result", "",
...
def main() -> int:
    ...
    result = {
        ...
        "commands": [
            f"python docs/tclk-pilot/generate_synthetic_fixture.py {args.database.name} --jobs {manifest['jobs']} --hot-extra-events {int(manifest['hot_task_events']) - 4}",
            f"python docs/tclk-pilot/phase1_verify.py --database {args.database.name} --manifest {args.manifest.name} --output-dir phase1-results --memory-mib {args.memory_mib} --timeout-seconds {args.timeout_seconds}",
        ],
    }
