...
SQLite close-time WAL checkpoints are disabled for short-lived request/write connections. The collector performs bounded-wait restart checkpoints every 25 catch-up observations and at partial-page boundaries so the existing WAL allocation can be reused, plus a passive checkpoint after each bounded poll cycle. It exposes the last result in health diagnostics. This avoids surprise checkpoints inside a request or single-observation commit while bounding transient WAL growth without truncating the database or WAL.
